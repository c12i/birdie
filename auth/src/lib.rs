...
pub async fn generate_oath1_token() -> Result<Oauth1aToken, anyhow::Error> {
    ...
    let api_key_secret = env::var("TWITTER_API_KEY_SECRET").context("$TWITTER_API_KEY_SECRET is not set")?;
    let access_token = env::var("TWITTER_API_ACCESS_TOKEN").context("$TWITTER_API_ACCESS_TOKEN is not set")?;
    let access_token_secret = env::var("TWITTER_API_ACCESS_TOKEN_SECRET").context("$TWITTER_API_ACCESS_TOKEN_SECRET is not set")?;
